...
import { MdDelete, MdSms } from "react-icons/md";
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      <div className="md:flex lg:flex-col gap-4">
        <div className="flex flex-col items-center justify-center bg-base-100 p-6 rounded-xl space-y-3 mb-6 shadow-xl border border-white hover:border-[#7fbea9]">
          ...
          <div>
            ...
            <div className="flex items-center justify-center text-white">
              <span
                className={`${efriend.status === "on-track" ? "bg-[#244D3F] py-1 px-2 ml-2 text-sm font-medium rounded-full" : ""}`}
              >
                {efriend.status === "on-track" ? "On Track" : ""}
              </span>
              <span
                className={`${efriend.status === "overdue" ? "bg-[#EF4444] py-1 px-2 ml-2 text-sm font-medium rounded-full" : ""}`}
              >
                {efriend.status === "overdue" ? "Overdue" : ""}
              </span>
              <span
                className={`${efriend.status === "almost due" ? "bg-[#EFAD44] py-1 px-2 ml-2 text-sm font-medium rounded-full" : ""}`}
              >
                {efriend.status === "almost due" ? "almost due" : ""}
              </span>
            </div>
            <div className="flex justify-center items-center my-4">
              {efriend.tags.map((tag) => (
                <span
                  className="bg-[#CBFADB] py-1 px-2 ml-2 rounded-3xl text-[12px] font-medium"
                  key={tag}
                >
                  {tag.toUpperCase()}
                </span>
        ...
        <div className="flex flex-col justify-center items-center gap-2">
          <button className="btn btn-wide bg-base-100">
      ...
      <div className="space-y-6">
        ...
        <div className="p-6 bg-base-100 rounded-2xl space-y-4 border-white shadow-md">
          <div className="flex justify-between">
            <h1 className="text-xl font-normal text-[#244D3F] ">
              Relationship Goal
            </h1>
          ...
          <p className="text-[#64748B] text-[18px] ">
            Connect every{" "}
            <span className="text-[#1F2937] font-bold text-[18px]">
              {efriend.goal} days
            </span>
          </p>
        </div>
        ...
        <div className="p-6 space-y-4 bg-base-100 rounded-2xl border-white shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 ">
            <div
              onClick={() => handleCall(efriend)}
              className="p-4 space-y-2 text-center bg-[#F8FAFC] rounded-lg flex flex-col items-center justify-center border border-[#E9E9E9] cursor-pointer hover:border-[#7fbea9]"
            >
              <PiPhoneCall />
              <p>Call</p>
            </div>
            <div
              onClick={() => handleSms(efriend)}
              className="p-4 space-y-2 text-center bg-[#F8FAFC] rounded-lg flex flex-col items-center justify-center border border-[#E9E9E9] cursor-pointer hover:border-[#7fbea9]"
            >
              <MdSms />
              <p>Text</p>
        ...
        <div className="bg-base-100 p-6 border-white shadow-md rounded-2xl">
          <div className="flex flex-wrap justify-between items-center">
            <h1 className="text-xl font-medium text-[#244D3F] mb-2 md:mb-0">
              Recent Interactions
            </h1>
          ...
          <div className="space-y-3 mt-4">
            {interactions.length > 0 ? (
              ...
              <p className="text-[#64748B]">
                No interactions yet. Use Call, Text, or Video to add one.
              </p>
            )}
          </div>
        </div>
